...
import { BORROW_COLOR, LEND_COLOR, directionColor } from '@/constants/borrowItems';
      ...
      <View style={styles.mapStage}>
        ...
        <View style={styles.summary}>
          <View style={styles.metric}>
            <Text style={styles.metricNumber}>{lendCount}</Text>
            <Text style={styles.metricLabel}>貸した</Text>
          </View>
          <View style={styles.metric}>
            <Text style={styles.metricNumber}>{borrowCount}</Text>
            <Text style={styles.metricLabel}>借りた</Text>
          </View>
          <View style={styles.metric}>
            <Text style={styles.metricNumber}>{activeItems.length}</Text>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { color: theme.text, fontSize: 24, fontWeight: '900', textAlign: 'center' },
  mapStage: { flex: 1, backgroundColor: '#0b111c', overflow: 'hidden' },
  horizontalScroll: { minHeight: 940 },
  verticalScroll: { width: 860, minHeight: 940, paddingHorizontal: 20, paddingVertical: 20 },
  canvas: { width: 820, height: 900, position: 'relative' },
  line: { position: 'absolute', height: 5, borderRadius: 999, transformOrigin: 'left center', opacity: 0.9 },
  meNode: { position: 'absolute', left: 366, top: 416, width: 88, height: 88, borderRadius: 44, alignItems: 'center', justifyContent: 'center', backgroundColor: '#ffd166' },
